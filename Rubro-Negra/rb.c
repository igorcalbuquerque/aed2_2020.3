...
void ajustar(arvore *raiz, arvore elemento){
	...
	while(cor(elemento->pai) == VERMELHO && cor(elemento) == VERMELHO) {
			...
			if(cor(tio(elemento)) == VERMELHO && cor(elemento->pai) == VERMELHO) {
		...
		if(valor == posicao->dado) {
				...
				if(posicao->esq == NULL && posicao->dir == NULL) {
					...
					break;
							...
							else if(eh_filho_esquerdo(posicao))
...
//		printf("caso 1\n");
	...
	if(cor(elemento->pai) == PRETO && cor(irmao(elemento)) == VERMELHO) {
				...
				if(eh_filho_esquerdo(elemento))//2a
	...
	if( cor(elemento->pai) == VERMELHO && cor(irmao(elemento)) == PRETO &&
		cor(irmao(elemento)->esq)== PRETO && cor(irmao(elemento)->dir) == PRETO) {	
			irmao(elemento)->cor = VERMELHO;
			elemento->pai->cor = PRETO;
	...
	if( eh_filho_esquerdo(elemento) && cor(irmao(elemento)) == PRETO &&
		cor(irmao(elemento)->esq) == VERMELHO && cor(irmao(elemento)->dir) == PRETO) {	
			irmao(elemento)->cor = VERMELHO;
	...
	//caso 5b
	if(!eh_filho_esquerdo(elemento) && cor(irmao(elemento)) == PRETO &&
		cor(irmao(elemento)->dir) == VERMELHO && cor(irmao(elemento)->esq) == PRETO) {	
				irmao(elemento)->cor = VERMELHO;
